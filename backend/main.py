...
# Route to fetch dustbin data (all dustbins)
@app.route("/dustbins", methods=["GET"])
def get_dustbins():
    dustbins = Dustbin.query.all()
    json_dustbins = list(map(lambda x: x.to_json(), dustbins))
...
# Route to update dustbin levels based on sensor data
@app.route("/update_dustbin_levels", methods=["POST"])
def update_dustbin_levels():
    try:
        sensor_data = request.json
        for data in sensor_data:
            dustbin = Dustbin.query.get(data['id'])
            if dustbin:
                dustbin.capacity = data['garbage_level']  # Update the dustbin's garbage level
                db.session.commit()
            else:
                return jsonify({"error": f"Dustbin with ID {data['id']} not found"}), 404
        
        return jsonify({"message": "Dustbin levels updated successfully"}), 200
    except Exception as e:
        print(f"Error updating dustbin levels: {str(e)}")
        return jsonify({"error": "Failed to update dustbin levels"}), 500

# Route to plan optimized route (with dynamic changes based on sensor data)
@app.route("/plan_optimized_route", methods=["POST"])
def plan_optimized_route_handler():
    try:
        dustbins_data = request.json.get("dustbins")
